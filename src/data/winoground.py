...
class WinogroundDataset(Dataset):
    ...
    def process_data(self, winoground):
        ...
        for example in winoground:
            ...
            e1 = {"image": example['image_1'], 'text': example['caption_1'], 'tag': example['tag'], 'secondary_tag': example['secondary_tag'], 'num_main_preds': example['num_main_preds'], 'collapsed_tag': example['collapsed_tag'], 'pair': len(data)}
            data.append(e0)
            data.append(e1)
